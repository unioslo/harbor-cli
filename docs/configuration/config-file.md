...
Controls whether or not the [harborapi](https://github.com/unioslo/harborapi) library  validates the data returned by the Harbor API.  Forces the output format to `json` if `false`. Not guaranteed to work with all commands. The default is `true`.
...
Controls whether or not the data from the API should be processed by the [harborapi](https://github.com/unioslo/harborapi) library before being returned. Overrides `harbor.validate_data`. Ignores output mode. Not guaranteed to work with all commands. The default is `false`.
