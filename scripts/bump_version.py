...
class VersionBumper(StateMachine):
    ...
    def git_add(self, *files: str) -> CompletedProcess[bytes]:
        p_git_add = self.run(
            ["git", "add", "harbor_cli/__about__.py", "CHANGELOG.md", *files],
            capture_output=True,
        )
    ...
    def add_changelog_header(self, new_version: str) -> None:
        ...
        header = f"## [{new_version}](https://github.com/unioslo/harbor-cli/tree/harbor-cli-v{new_version}) - {datetime.now().strftime('%Y-%m-%d')}"
